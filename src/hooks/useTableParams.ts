...
export function usePaginationParams(defaults?: {
  ...
  const pageSize = parseInt(
    searchParams.get("pageSize") || defaults?.pageSize?.toString() || TABLE_PAGE_SIZE.toString(),
    10
  );
  ...
  const sortType =
    (searchParams.get("sortType") as "asc" | "desc") || defaults?.sortType || DEFAULT_SORT_TYPE;
